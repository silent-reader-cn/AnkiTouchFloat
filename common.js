...
(function () {
    // 如果出现换牌无效请检查是否引入了两次common.js
    var setFront = () => {
        document.documentElement.classList.add('common_front')
        document.documentElement.classList.remove('common_back')
    };
    var setBack = () => {
        document.documentElement.classList.add('common_back')
        document.documentElement.classList.remove('common_front')
    };

    //利用只有换牌时刷新前后的机制检测前后 
    if (!document.documentElement.classList.contains('common_front') && !document.documentElement.classList.contains('common_back'))
        setFront(), console.log("首次启动重置为FRONT状态");
    else {
        if (document.documentElement.classList.contains('common_front'))
            setBack();
        else
            setFront();
    }

    //if(document.querySelectoAll("script[src='common.js']")?.length??0>1)
    ...
    if (window.answerMode === undefined) {
        Object.defineProperty(window, "answerMode", {
            get: () => document.documentElement.classList.contains('common_front')
        })
